## ----------------------------------------------------------------------------------------------------- ## 
## File:     dda_resdist.R 
## Purpose:  Source code for the residual distribution DDA component 
...
# - - - Changes: 
# (1) Implemented coskewness and cokurtosis difference tests (rescaled to ensure positive difference under the target model) 
# (2) Implemented LR approximations (see dda.vardist(...))
# (3) squared higher moments are used to compute differences instead of absolute values due to interpretative reasons
# (4) residuals are standardized prior DDA 
# (5) skew.diff.test and kurt.diff.test no longer reports absolute z-scores.
# (6) option prob.int added. If prop.int = TRUE, a probability integral transformation is performed prior 
#     computetion of skewness and kurtosis difference tests in case errors are non-normal. 
# - - - 
...
dda.resdist <- function(formula, pred = NULL, data = list(), B = 100, boot.type = "bca", conf.level = 0.95, prob.trans = FALSE){
...
##             are taken from the environment which dda.resdist is called from)
...
## prob.trans: A logical evaluating to TRUE or FALSE indicating whether a probability integral transformation 
##             should be performed prior computation of skewness and kurtosis difference tests.  
## Example:    dda.resdist(y ~ x + z, pred = "x", data = my.data) or 
##             m <- y ~ x + z 
##             dda.resdist(m, pred = "x", data = my.data)
## -----------------------------------------------------------------------------------------------------------
   ...
   if(B > 0){
             suppressWarnings(boot.res <- boot(dat,  , R = B, prob.trans = prob.trans))
